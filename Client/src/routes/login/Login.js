    ...
    const submit_connection = (e) => {
        ...
        for (let i = 0; i < testNode.length; i++) {
            if (test !== 1) {
                ...
                axios.post("https://lorenzo-geano-sport-app.herokuapp.com/API/insertPseudo", { Username: Username, Mdp: Mdp })
    ...
    const submit_verif = () => {
        ...
        for (let i = 0; i < testNode.length; i++) {
            ...
            if (testNode[i].Username === Username_login && testNode[i].Mdp === Mdp_login) {
                ...
                axios.post("https://lorenzo-geano-sport-app.herokuapp.com/API/insertPseudo", { Username: Username_login })
                    .then(() => { alert("inset succes") });
